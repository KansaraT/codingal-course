...
SELECT customer.cust_name, salesman.name, salesman.city
FROM Customer
LEFT JOIN Salesman ON Customer.city = Salesman.city;

--sqlite does not support outer join and right join. Use union to perforn outer join.

-- Linking customers to their salesmen
SELECT Customer.customer_id, Customer.cust_name, Salesman.name FROM Customer
...
SELECT Orders.ord_no, Customer.cust_name, Orders.customer_id, Orders.Salesman_id FROM Orders
